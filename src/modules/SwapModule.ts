...
export class SwapModule implements IModule {
  ...
  async isPairExist(coinX: AptosResourceType, coinY: AptosResourceType): Promise<boolean> {
    ...
    const lpType = composeLP(modules.Scripts, isSorted ? coinX : coinY, isSorted ? coinY : coinX)
    try {
  ...
  addLiquidityPayload({
    ...
    newAmountX = d(newAmountX)
    newAmountY = d(newAmountY)
    slippage = d(slippage)
  ...
  async removeLiquidityRates({
    ...
    const { modules } = this.sdk.networkOptions
    const isSorted = isSortedSymbols(coinX, coinY)
    const lpCoin = composeLPCoin(modules.ResourceAccountAddress, isSorted ? coinX : coinY, isSorted ? coinY : coinX)
    const lpType = composeLP(modules.Scripts, isSorted ? coinX : coinY, isSorted ? coinY : coinX)
  ...
  removeLiquidityPayload({
    ...
    amount = d(amount)
    amountXDesired = d(newAmountXDesired)
    amountYDesired = d(newAmountYDesired)
    slippage = d(slippage)
